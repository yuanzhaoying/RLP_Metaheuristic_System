...
def crossover_order(parent1: List[int], parent2: List[int], rng: RandomGenerator) -> Tuple[List[int], List[int]]:
    n = len(parent1)
    if len(parent2) != n or n < 2:
        return parent1.copy(), parent2.copy()
    
    i, j = sorted([rng.integers(0, n), rng.integers(0, n)])
    ...
    def fill_child(child, segment, other_parent):
        ...
        remaining = [x for x in other_parent if x not in segment_set]
    ...
    child1 = fill_child(child1, parent1[i:j+1], parent2)
    child2 = fill_child(child2, parent2[i:j+1], parent1)
