...
class FeatureExtractor:
    ...
    def _structural_features(self) -> Dict[str, float]:
        ...
        # 排除虚活动（第0个和最后一个）
        real_activities = list(range(1, n - 1)) if n > 2 else list(range(n))
        n_real = len(real_activities)

        # 只计算真实活动的边数
        n_edges = 0
        for i in real_activities:
            for j in self.inst.successors[i]:
                if j in real_activities or j == n - 1:  # 包含指向结束虚活动的边
                    n_edges += 1

        out_degrees = [len([s for s in self.inst.successors[i] if s in real_activities or s == n - 1]) 
                      for i in real_activities]
        in_degrees = [len([p for p in self.inst.predecessors[i] if p in real_activities or p == 0]) 
                     for i in real_activities]
        ...
        parallel_degree = n_edges / max(1, n_real * (n_real - 1) / 2)

        depth = 0
        for node in real_activities:
            if len(self.inst.predecessors[node]) == 0 or all(p == 0 for p in self.inst.predecessors[node]):
                longest_path = 0
                for successor in self.inst.successors[node]:
        ...
        return {
            ...
            "max_out_degree": max(out_degrees) if out_degrees else 0.0,
            "max_in_degree": max(in_degrees) if in_degrees else 0.0,
            "critical_path_len": critical_path,
            "order_strength": order_strength,
            "parallel_degree": parallel_degree,
            "density": n_edges / max(1, n_real * (n_real - 1) / 2),
        }
    ...
    def _resource_features(self) -> Dict[str, float]:
        ...
        # 只使用真实活动的需求和持续时间
        real_demands = demands[real_activities]
        real_durations = durations[real_activities]

        total_work = (real_demands.T * real_durations).sum(axis=1)
        ...
        demand_per_activity = real_demands.sum(axis=1)
        ...
        # 计算resource_factor和resource_usage
        b = []
        for i in range(n):
            for r in range(self.inst.n_resources):
                if demands[i, r] > 0:
        ...
        RF = sum(b) / max(1, (n - 2) * self.inst.n_resources) if n > 2 else 0
        RU = sum(b) / max(1, n - 2) if n > 2 else 0

        # 计算每个资源的平均需求
        total_resource_request = real_demands.sum(axis=0)
        avg_resource_request = total_resource_request / max(1, len(real_activities))
        ...
        features = {
            ...
            "demand_mean": float(real_demands.mean()),
            "demand_std": float(real_demands.std()),
            ...
            "resource_factor": float(RF),
            "resource_usage": float(RU),
            "avg_resource_total": float(np.mean(avg_resource_request)),
        }
    ...
    def _slack_features(self) -> Dict[str, float]:
        ...
        if len(es) == 0 or len(real_activities) == 0:
            return {
                "slack_mean": 0.0,
                "slack_std": 0.0,
                "slack_min": 0.0,
        ...
        # 只使用真实活动的松弛时间
        real_slacks = ls[real_activities] - es[real_activities]
        non_zero_slacks = real_slacks[real_slacks > 0]

        critical_count = np.sum(real_slacks == 0)
        critical_ratio = critical_count / len(real_slacks) if len(real_slacks) > 0 else 0.0

        return {
            "slack_mean": float(real_slacks.mean()),
            "slack_std": float(real_slacks.std()) if len(real_slacks) > 1 else 0.0,
            "slack_min": float(real_slacks.min()),
            "slack_max": float(real_slacks.max()),
            "slack_median": float(np.median(real_slacks)),
    ...
    def _network_topology_features(self) -> Dict[str, float]:
        ...
        # 只使用真实活动的持续时间
        real_durations = durations[real_activities]
        features["duration_mean"] = float(np.mean(real_durations))
        features["duration_max"] = float(np.max(real_durations))
        features["duration_min"] = float(np.min(real_durations))

        # 只计算真实活动的前驱和后继数量
        predecessor_counts = [len([p for p in self.inst.predecessors[i] if p in real_activities or p == 0]) 
                             for i in real_activities]
        successor_counts = [len([s for s in self.inst.successors[i] if s in real_activities or s == n - 1]) 
                           for i in real_activities]

        features["max_predecessor_count"] = float(max(predecessor_counts)) if predecessor_counts else 0.0
        features["min_predecessor_count"] = float(min(predecessor_counts)) if predecessor_counts else 0.0
        features["max_successor_count"] = float(max(successor_counts)) if successor_counts else 0.0
        features["min_successor_count"] = float(min(successor_counts)) if successor_counts else 0.0

        # 只计算真实活动之间的边数
        n_edges = sum(successor_counts)
        features["network_complexity"] = n_edges / max(1, n_real)

        # 计算order_strength时排除虚活动的边
        first_successor_count = len([s for s in self.inst.successors[0] if s in real_activities])
        last_predecessor_count = len([p for p in self.inst.predecessors[n - 1] if p in real_activities])
        order_strength_new = (n_edges - first_successor_count - last_predecessor_count) / max(1, n_real * (n_real - 1) / 2)
