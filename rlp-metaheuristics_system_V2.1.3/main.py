...
from src.eval.stats_layerB import build_layerB_df, mixedlm_factor_lrt, art_anova
...
J30_COUNT = 1      # j30实例数量 (总共480个)
J60_COUNT = 0      # j60实例数量 (总共480个)
J90_COUNT = 0      # j90实例数量 (总共480个)
J120_COUNT = 0     # j120实例数量 (总共600个)
...
# 编码类型: "start_time", "activity_list" 或 "shift_vector"
# - "start_time": 开始时间编码（默认）
# - "activity_list": 活动列表编码（AL编码）
# - "shift_vector": 基于位移的编码（SV编码）
'''
# 生成的解是否满足约束条件
# 用小算例验证
...
# 随机种子配置
# - USE_RANDOM_SEEDS: 是否使用随机种子
#   - True: 使用SEEDS中指定的种子列表
#   - False: 不使用随机种子（设置为None或空列表）
# - SEEDS: 种子列表，#   - 当USE_RANDOM_SEEDS=True时，使用此列表中的种子
#   - 当USE_RANDOM_SEEDS=False时，此列表被忽略
...
def run_experiments(config: dict, instances, deadlines, instance_files, use_delay_factors: bool = False, encoding_type: str = "start_time", seeds = None):
    ...
    if None in actual_seeds:
        print(f"  - Seeds: 不使用随机种子")
    else:
    ...
    print(f"  - Encoding type: {exp_config.encoding_type}")

    runner = ExperimentRunner(exp_config)
    
    if encoding_type == "activity_list":
        print(f"\n  AL Encoding Algorithm configurations:")
        print(f"    - GA: 384 combinations (2 selection × 3 crossover × 4 mutation × 2 initialization × 2 repair × 2 elitism × 2 local_search)")
        print(f"    - DE: 48 combinations (6 mutation × 2 crossover × 2 initialization × 2 local_search)")
        print(f"    - PR: 16 combinations (4 path × 2 selection × 2 local_search)")
        print(f"    - TS: 2 combinations (2 tabu_strategy)")
        print(f"    - PSO: 4 combinations (2 local_search × 2 restart)")
        print(f"    - BA: 2 combinations (2 local_search)")
        print(f"    - HS: 4 combinations (2 parameter × 2 initialization)")
        print(f"    - SA: 3 combinations (3 neighborhood)")
        print(f"    - Total: 463 algorithm configurations")
    elif encoding_type == "shift_vector":
        print(f"\n  SV Encoding Algorithm configurations:")
        print(f"    - GA: 384 combinations (2 selection × 3 crossover × 4 mutation × 2 initialization × 2 repair × 2 elitism × 2 local_search)")
        print(f"    - DE: 48 combinations (6 mutation × 2 crossover × 2 initialization × 2 local_search)")
        print(f"    - PR: 16 combinations (4 path × 2 selection × 2 local_search)")
        print(f"    - TS: 2 combinations (2 tabu_strategy)")
        ...
        print(f"\n  Start Time Encoding Algorithm configurations:")
        print(f"    - DE: 80 combinations (10 mutation strategies × 4 crossover strategies × 2 local search)")
        print(f"    - GA: many combinations")
        print(f"    - SA: 1 combination")
        print(f"    - PSO: 4 combinations")
        print(f"    - BA: 2 combinations")
        print(f"    - HS: 4 combinations")
    ...
    filename = dt_now.now().strftime('%m%d_%H%M') + f"_{encoding_type}" + ".csv"
    output_dir = "results/raw"
...
def main():
    ...
    parser.add_argument("--encoding", default=ENCODING_TYPE, choices=["start_time", "activity_list", "shift_vector"], help="Encoding type: start_time, activity_list or shift_vector")
    parser.add_argument("--seeds", type=int, nargs="+", default=None, help="Random seeds to use (space-separated, e.g., --seeds 0 1 2)")
    parser.add_argument("--no-random-seeds", action="store_true", default=False, help="Disable random seeds (run without seeds)")

    args = parser.parse_args()
    
    if args.no_random_seeds:
        actual_seeds = None
    ...
    print(f"  - 编码类型: {args.encoding}")
    
    if actual_seeds is None:
        print(f"  - 随机种子: 不使用")
    else:
        print(f"  - 随机种子: {actual_seeds}")
    print()
    ...
    if not args.skip_experiments:
        ...
        results_df = run_experiments(config, instances, deadlines, instance_files, args.use_delay_factors, args.encoding, actual_seeds)
    else:
