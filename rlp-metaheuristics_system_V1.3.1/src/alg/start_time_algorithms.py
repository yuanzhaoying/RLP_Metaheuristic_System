...
def create_algorithm_st(algo_name: str, instance: RCPSPInstance, deadline: int, params):
    ...
    if algo_name == "ga":
        ...
        from .PSO import ParticleSwarmOptimizationST
        return ParticleSwarmOptimizationST(instance, deadline, params), "start_time"
    elif algo_name == "ba":
        from .BA import BatAlgorithmST
        return BatAlgorithmST(instance, deadline, params), "start_time"
    elif algo_name == "hs":
        from .HS import HarmonySearchST
        return HarmonySearchST(instance, deadline, params), "start_time"
    else:
        raise ValueError(f"Unknown algorithm: {algo_name}. Available: ga, sa, ils, ts, pr, gsa, de, pso, ba, hs")
