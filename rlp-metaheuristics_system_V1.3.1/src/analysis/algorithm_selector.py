...
class AlgorithmSelector:
    ...
    def analyze_feature_importance(self):
        ...
        if hasattr(self.model, 'feature_importances_'):
            ...
            # 对于SVM等没有feature_importances_的模型，使用置换重要性
...
def analyze_selector(perf_df, feature_df, model_type='decision_tree', 
    ...
    # 第四阶段：解释性分析
    feature_importance = selector.analyze_feature_importance()
    embedding_df, isa_fig = selector.instance_space_analysis(X, y, method='tsne' if use_tsne else 'pca')
    
    # 特征重要性图
    fi_fig = selector.plot_feature_importance(top_n=20)
