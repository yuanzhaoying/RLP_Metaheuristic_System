...
def generate_all_algorithm_configs():
    ...
    local_search_strategies = ["none", "activity", "shift"]  # 修改：三种局部搜索策略
    neighborhood_options = [True, False]
    sa_acceptance_options = [True, False]  # 修改：bool类型的SA接受准则
                    ...
                    for ls_strategy in local_search_strategies:
                        for use_neighborhood in neighborhood_options:
                            for use_sa in sa_acceptance_options:
                                config_name = f"GA_{sel}_{cross}_{mut}_{init}_{ls_strategy}_{'withNeighborhood' if use_neighborhood else 'noNeighborhood'}_{'withSA' if use_sa else 'noSA'}"
    ...
    # PSO算法（4种组合）
    local_search_strategies = ["none", "shift"]
